(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    // This is array for calculating average Expression 

    function drawFaceExpressions(canvasArg, faceExpressions,Names, minConfidence, textFieldAnchor) {
        if (minConfidence === void 0) { minConfidence = 0.5; }
        var i=0;
        ...
        faceExpressionsArray.forEach(function (e) {
           ...
           var drawTextField = new DrawTextField(resultsToDisplay.map(function (expr) { 
                test(expr.expression,Names[i])
            i++;
                
               return expr.expression + " (" + round(expr.probability) + ")"; }), anchor);
            drawTextField.draw(canvasArg);
        
            // console.log('I am array:= '+ ExpressionArray +"length : "+ ExpressionArray.length)

        });
    }
        //this is the copy of drawFaceExpression
        
        function drawFaceExpressions1(canvasArg, faceExpressions, minConfidence, textFieldAnchor) {
            if (minConfidence === void 0) { minConfidence = 0.5; }
            // var i=0;
            var faceExpressionsArray1 = Array.isArray(faceExpressions) ? faceExpressions : [faceExpressions];
            faceExpressionsArray1.forEach(function (e) {
                var expr1 = e instanceof FaceExpressions
                    ? e
                    : (isWithFaceExpressions(e) ? e.expressions : undefined);
                if (!expr1) {
                    throw new Error('drawFaceExpressions - expected faceExpressions to be FaceExpressions | WithFaceExpressions<{}> or array thereof');
                ...
                var sorted1 = expr1.asSortedArray();
                var resultsToDisplay1 = sorted.filter(function (expr) { return expr.probability > minConfidence; });
                var anchor1 = isWithFaceDetection(e)
                    ? e.detection.box.bottomLeft
                    : (textFieldAnchor || new Point(0, 0));
    
                var drawTextField1 = new DrawTextField(resultsToDisplay.map(function (expr1) { 
                    
                    // test(expr.expression,Names[i])
                // i++;
                    return expr1.expression + " (" + round(expr1.probability) + ")"; }), anchor1);
                drawTextField.draw(canvasArg);
                // console.log('I am array:= '+ ExpressionArray +"length : "+ ExpressionArray.length)
    ...
    var drawExtended = /*#__PURE__*/Object.freeze({
        ...
        drawFaceExpressions1: drawFaceExpressions1,
